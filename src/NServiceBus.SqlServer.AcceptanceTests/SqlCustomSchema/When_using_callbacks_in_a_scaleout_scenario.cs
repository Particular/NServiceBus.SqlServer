    ...
    public class When_using_callbacks_in_a_scaleout_scenario : NServiceBusAcceptanceTest
    {
        ...
        public void Each_client_should_have_a_unique_input_queue_to_avoid_processing_each_others_callbacks()
        {
            Scenario.Define(() => new Context{Id = Guid.NewGuid()})
                    ...
                    {
                        b.CustomConfig(c => RuntimeEnvironment.MachineNameAction = () => "ClientA");
                    ...
                    .WithEndpoint<Client>(b =>
                    {
                        b.CustomConfig(c => RuntimeEnvironment.MachineNameAction = () => "ClientB");
                        b.Given((bus, context) => bus.Send(new MyRequest
                        {
                            Id = context.Id,
                            Client = RuntimeEnvironment.MachineName
        ...
        public class Server : EndpointConfigurationBuilder
        {
            public Server()
            {
                EndpointSetup<DefaultServer>( c => c.UseTransport<SqlServer>( ConnectionStringWith_test_schema ) );
            }
