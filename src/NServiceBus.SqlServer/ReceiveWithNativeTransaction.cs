namespace NServiceBus.Transports.SQLServer
{
    ...
    class ReceiveWithNativeTransaction : ReceiveStrategy
    {
        ...
        public async Task TryReceiveFrom(TableBasedQueue inputQueue, TableBasedQueue errorQueue, Func<PushContext, Task> onMessage)
        {
            using (var sqlConnection = new SqlConnection(this.connectionString))
            {
                ...
                using (var transaction = sqlConnection.BeginTransaction())
                {
                    try
                    {
                        ...
                        readResult = inputQueue.TryReceive(sqlConnection, transaction);
