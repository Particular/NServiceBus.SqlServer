...
public class ConfigureEndpointSqlServerTransport : IConfigureEndpointTestExecution
{
    ...
    public ConfigureEndpointSqlServerTransport()
    {
        ...
        //On Linux we need to explicitly set the transaction mode to SendsAtomicWithReceive since TransactionScope is not available there
        if (OperatingSystem.IsLinux() && transport.TransportTransactionMode == TransportTransactionMode.TransactionScope)
        {
